...
export const useDocuments = () => {
  ...
  const markObsoleteMutation = useMutation({
    ...
    onError: (error) => {
      toast({
        title: 'Errore',
        description: `Impossibile rendere obsoleto il documento: ${error instanceof Error ? error.message : 'Errore sconosciuto'}`,
        variant: 'destructive'
      });
    }
  });
  ...
  const updateDocumentMutation = useMutation({
    ...
    onError: (error) => {
      toast({
        title: 'Errore',
        description: `Impossibile aggiornare il documento: ${error instanceof Error ? error.message : 'Errore sconosciuto'}`,
        variant: 'destructive'
      });
    }
  });
