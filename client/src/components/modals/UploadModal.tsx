      ...
      <DialogContent>
        ...
        <div className="mt-2">
          <p className="text-sm text-gray-500 mb-4">
            Seleziona i file da caricare. Il sistema processerà automaticamente i documenti seguendo il formato stabilito
            (es. "4.2-Sicurezza Alimentare-Rev.1-20250325").
          </p>
          ...
          >
            <div className="flex flex-col justify-center items-center pt-5 pb-6">
              <span className="material-icons-round text-gray-400 mb-3" style={{ fontSize: '3rem' }}>cloud_upload</span>
              <p className="mb-2 text-sm text-gray-500">
                <span className="font-semibold">Clicca per selezionare</span> o trascina i file
              </p>
              <p className="text-xs text-gray-500">I formati supportati sono Excel, Word e PDF</p>
            </div>
            <input {...getInputProps()} />
          </div>
          ...
          {files.length > 0 && (
            <div className="mt-4">
              <h4 className="text-sm font-medium text-gray-900 mb-2">File selezionati ({files.length})</h4>
                  ...
                  <li key={index} className="py-2 flex items-center">
                    <span className="material-icons-round text-primary-600 mr-2 text-sm">
                      {file.type.includes('pdf') ? 'picture_as_pdf' : 
                       file.type.includes('word') ? 'article' : 'description'}
                    </span>
                    {file.name}
                  </li>
                ))}
          ...
          {uploadMutation.isPending && (
            <div className="mt-4">
              <p className="text-sm text-gray-500 mb-2">Caricamento in corso... {uploadProgress}%</p>
